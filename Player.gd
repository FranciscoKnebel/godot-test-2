...
const JUMP_SPEED = 25
const ACCEL = 4.5
const DEACCEL= ACCEL*10
const MAX_SLOPE_ANGLE = 40

const MAX_SPRINT_SPEED = MAX_SPEED * 1.5
const SPRINT_ACCEL = ACCEL * 4
...
func process_movement(delta):
	...
	var accel
	if dir.dot(hvel) > 0:
		accel = SPRINT_ACCEL if is_sprinting else ACCEL
	else:
		accel = DEACCEL
...
func _input(event):
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		camera_rot.x = clamp(camera_rot.x, -60, 80)
